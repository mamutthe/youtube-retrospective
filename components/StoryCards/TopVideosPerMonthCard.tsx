...
export function TopVideosPerMonthCard() {
  ...
  return (
    <BaseCard className="flex flex-col items-center justify-center space-y-1 bg-gradient-to-r from-yellow-300 to-rose-500 px-4 py-1 text-center">
      <span className="mb-1 text-lg font-medium text-white md:font-bold">
      ...
      {Object.values(topVideosPerMonth).map((video, index: number) => (
        <StoryInfoCard
          ...
          className="flex h-[2.3rem] flex-row md:h-[2rem]"
          isNotHFull={true}
          textStyle={
        ...
        >
          ...
          <ViewCount className="order-3 ml-auto mr-0 rounded-l-none rounded-r-xl bg-rose-500/10 text-sm leading-5 text-rose-700">
            {video[0].views}
          </ViewCount>
        </StoryInfoCard>
      ))}
